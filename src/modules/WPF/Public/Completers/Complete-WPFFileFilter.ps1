...
#>
function Complete-WPFFileFilter {
    [CmdletBinding()]
    ...
    $Completions = foreach ($KVP in $script:WPFFileInfoCache.GetEnumerator()) {
        $Category, $FileInfos = $KVP.Name, $KVP.Value

        $FileInfos |
            Where-Object {
                $_.Name.StartsWith($WordToComplete, [StringComparison]::InvariantCultureIgnoreCase)
            } |
            Sort-Object -Property Name |
            ForEach-Object {
                # Complete Filter
                $CompletionText = "{0}{1} {2}{3}" -f $Quote, $_.Name, $_.Filter, $Quote

                [CompletionResult]::new(
                    <# Text to insert #> $CompletionText,
                    <# Text displayed in the list #> $_.Name,
