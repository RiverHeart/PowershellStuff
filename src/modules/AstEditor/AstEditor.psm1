...
function Test-AstDocumentInstance {
    param (
        [Parameter(Mandatory)]
        [object] $InputObject
    )

    return $InputObject -is [AstDocument]
}

function New-AstDocumentInstance {
    param (
        [Parameter(Mandatory)]
        [string] $Path,
    ...
    return [AstDocument]::new($Path, $Text, $Ast, $Tokens, $ParseErrors)
}
